 ...
 * <p/>
 * BIP 32 supports a tree hierarchy of keys, and ykneo-bitcoin supports accessing these sub keys for use in the
 * getPublicKey and sign methods. Private key derivation is supported, by setting the first (sign-) bit of index, as
 * per the BIP 32 specification.
 * <p/>
 * Example:
 * YkneoBitcoin neo = ...
 * byte[] extendedPrivateMasterKey m = ...
 * neo.importExtendedKeyPair(m, false); //neo now holds the master key pair m.
 * <p/>
 * neo.getPublicKey(0, 7); //This returns the uncompressed public key from sub key m/0/7
 * neo.sign(1, 4711 | 0x80000000, hash); //This returns the signature of hash signed by m/1/4711'
 */
